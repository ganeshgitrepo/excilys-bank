(function(jQuery, undefined) {
	...
	jQuery.extend({
		...
		template : function(name, tmpl) {
			...
			// If not in map, and not containing at least on HTML tag, treat as
			// a selector.
	...
	jQuery.extend(jQuery.tmpl, {
		tag : {
			...
			// tmpl target parameter can be of type function, so use $1, not $1a
			// (so not auto detection of functions)
			// This means that {{tmpl foo}} treats foo as a template (which IS a
			// function).
			// Explicit parens can be used if foo is a function that returns a
			// template: {{tmpl foo()}}.
			},
			"wrap" : {
				_default : {
	...
	function build(tmplItem, nested, content) {
		...
		var frag, ret = content ? jQuery.map(content, function(item) {
			...
			// Insert template item annotations, to be converted to jQuery.data(
			// "tmplItem" ) when elems are inserted into DOM.
				...
				jQuery.trim(markup).replace(/([\\'])/g, "\\$1").replace(/[\r\t\n]/g, " ").replace(/\$\{([^\}]*)\}/g, "{{= $1}}").replace(
						...
						function(all, slash, type, fnargs, target, parens, args) {
							...
							if (target) {
								...
								// Support for target being things like
								// a.toLowerCase();
								// In that case don't call with template item as
								// 'this' pointer. Just evaluate...
							...
							return "');"
									+ tag[slash ? "close" : "open"].split("$notnull_1").join(target ? "typeof(" + target + ")!=='undefined' && (" + target + ")!=null" : "true")
											.split("$1a").join(exprAutoFnDetect).split("$1").join(expr).split("$2").join(fnargs || def.$2 || "") + "__.push('";
						}) + "');}return __;");
	}
	function updateWrapped(options, wrapped) {
		// Build the wrapped content.
		options._wrap = build(options, true,
		// Suport imperative scenario in which options.wrapped can be set to a
		// selector or an HTML string.
