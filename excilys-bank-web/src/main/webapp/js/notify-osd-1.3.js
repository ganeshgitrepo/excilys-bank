		...
		create : function(options) {
			...
			notif_obj.extend({
				...
				set_click_through : function(click_through) {
					$('.clone').remove();
					if (click_through) {
						$('a,input').not('.notify-osd-dismiss').each(
								function() {
									var link = {
									...
									if (link.top_left.lies_inside(notification) || link.top_right.lies_inside(notification) || link.bottom_right.lies_inside(notification)
											|| link.bottom_left.lies_inside(notification)) {
				...
				show : function() {
					...
					$(this).fadeIn('fast', function() {
						...
						this.timeout = (!opts.sticky && opts.timeout) ? setTimeout(function() {
							notif_obj.dismiss();
						}, opts.timeout * 1000) : null;
						mouse = new Point(0, 0);
