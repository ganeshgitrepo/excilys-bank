 ...
 * {@link Introspector#getBeanInfo(Class)}) by including non-void returning
 * setter methods in the collection of {@link #getPropertyDescriptors() property
 * descriptors}. Both regular and <a href=
 * "http://download.oracle.com/javase/tutorial/javabeans/properties/indexed.html"
...
class ExtendedBeanInfo implements BeanInfo {
	...
	public ExtendedBeanInfo(BeanInfo delegate) throws IntrospectionException {
		...
		// PropertyDescriptor instances from the delegate object are never added
		// directly, but always
		// copied to the local collection of #propertyDescriptors and returned
		// by calls to
		// #getPropertyDescriptors(). this algorithm iterates through all
		// methods (method descriptors)
		// in the wrapped BeanInfo object, copying any existing
		// PropertyDescriptor or creating a new
		...
		ALL_METHODS: for (MethodDescriptor md : delegate.getMethodDescriptors()) {
			...
			// is the method a NON-INDEXED setter? ignore return type in order
			// to capture non-void signatures
			if (method.getName().startsWith("set") && method.getParameterTypes().length == 1) {
				...
				for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
					...
					// has the setter already been found by the wrapped
					// BeanInfo?
					if (writeMethod != null && writeMethod.getName().equals(method.getName())) {
						// yes -> copy it, including corresponding getter method
						// (if any -- may be null)
						this.addOrUpdatePropertyDescriptor(pd, propertyName, readMethod, writeMethod);
						continue ALL_METHODS;
					}
					// has a getter corresponding to this setter already been
					// found by the wrapped BeanInfo?
				...
				// the setter method was not found by the wrapped BeanInfo ->
				// add a new PropertyDescriptor for it
				// no corresponding getter was detected, so the 'read method'
				// parameter is null.
				this.addOrUpdatePropertyDescriptor(null, propertyName, null, method);
				continue ALL_METHODS;
			}

			// is the method an INDEXED setter? ignore return type in order to
			// capture non-void signatures
			if (method.getName().startsWith("set") && method.getParameterTypes().length == 2 && method.getParameterTypes()[0].equals(int.class)) {
				...
				DELEGATE_PD: for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
					...
					// has a getter corresponding to this setter already been
					// found by the wrapped BeanInfo?
					if (indexedReadMethod != null && indexedReadMethod.getName().equals(getterMethodNameFor(propertyName))
				...
				// the INDEXED setter method was not found by the wrapped
				// BeanInfo -> add a new PropertyDescriptor
				// for it. no corresponding INDEXED getter was detected, so the
				// 'indexed read method' parameter is null.
			...
			for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
				// have we already copied this read method to a property
				// descriptor locally?
				...
				if (method.equals(pd.getReadMethod()) || (pd instanceof IndexedPropertyDescriptor && method == ((IndexedPropertyDescriptor) pd).getIndexedReadMethod())) {
					// yes -> copy it, including corresponding setter method (if
					// any -- may be null)
					if (pd instanceof IndexedPropertyDescriptor) {
						this.addOrUpdatePropertyDescriptor(pd, pd.getName(), pd.getReadMethod(), pd.getWriteMethod(), ((IndexedPropertyDescriptor) pd).getIndexedReadMethod(),
	...
	private void addOrUpdatePropertyDescriptor(PropertyDescriptor pd, String propertyName, Method readMethod, Method writeMethod, Method indexedReadMethod,
		...
		for (PropertyDescriptor existingPD : this.propertyDescriptors) {
			if (existingPD.getName().equals(propertyName)) {
				// is there already a descriptor that captures this read method
				// or its corresponding write method?
				if (existingPD.getReadMethod() != null) {
					if (readMethod != null && existingPD.getReadMethod().getReturnType() != readMethod.getReturnType() || writeMethod != null
							&& existingPD.getReadMethod().getReturnType() != writeMethod.getParameterTypes()[0]) {
				...
				if (readMethod != null) {
					try {
						...
						// there is a conflicting setter method present -> null
				...
				// is there already a descriptor that captures this write method
				// or its corresponding read method?
				if (existingPD.getWriteMethod() != null) {
					if (readMethod != null && existingPD.getWriteMethod().getParameterTypes()[0] != readMethod.getReturnType() || writeMethod != null
							&& existingPD.getWriteMethod().getParameterTypes()[0] != writeMethod.getParameterTypes()[0]) {
						// no -> add a new descriptor for it below
						break;
					}
				}
				...
				if (existingPD instanceof IndexedPropertyDescriptor) {
					...
					// is there already a descriptor that captures this indexed
					// read method or its corresponding indexed write method?
					if (existingIPD.getIndexedReadMethod() != null) {
						if (indexedReadMethod != null && existingIPD.getIndexedReadMethod().getReturnType() != indexedReadMethod.getReturnType() || indexedWriteMethod != null
								&& existingIPD.getIndexedReadMethod().getReturnType() != indexedWriteMethod.getParameterTypes()[1]) {
							// no -> add a new descriptor for it below
							break;
						}
					}
					...
					try {
						...
						// there is a conflicting indexed setter method present
						// -> null it out and try again
						existingIPD.setIndexedWriteMethod(null);
						existingIPD.setIndexedReadMethod(indexedReadMethod);
					}

					// is there already a descriptor that captures this indexed
					// write method or its corresponding indexed read method?
		...
		// we haven't yet seen read or write methods for this property -> add a
		// new descriptor
		if (pd == null) {
			try {
				...
				logger.warn(format("Could not create new PropertyDescriptor for readMethod [%s] writeMethod [%s] "
						+ "indexedReadMethod [%s] indexedWriteMethod [%s] for property [%s]. Reason: %s", readMethod, writeMethod, indexedReadMethod, indexedWriteMethod,
			...
			try {
				...
				logger.warn(format("Could not add write method [%s] for property [%s]. Reason: %s", writeMethod, propertyName, ex.getMessage()));
